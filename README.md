...
The current API exposes `/api/v1/health`, `/api/v1/skills`, `/api/v1/skills/sync`, `/api/v1/careers`, `/api/v1/user/career-profile`, `/api/v1/jobs/feed`, and `/api/v1/ai/hub/process`. The job feed is seeded demo data until provider-specific adapters, terms-of-use checks, deduplication, and rate limits are implemented. Do not scrape sites that prohibit it; prefer official APIs and employer feeds.
